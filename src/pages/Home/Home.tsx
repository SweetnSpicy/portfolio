...
const Card = ({ name, title, desc }: { name: string; title: string; desc: string }) => (
  <div className="card">
...
const Button = ({ label, onClick }: { label: string; onClick: () => void }) => (
  <button onClick={onClick} className="bg-terracotta text-cream px-6 py-3 rounded">
    {label}
  </button>
);
...
const Home = () => {
  ...
  const [flightDirection, setFlightDirection] = useState<'left' | 'right'>('left');
  const navigate = useNavigate();

  const handleClick = (user: string, page: string) => {
    setUserName(user);
  ...
  return (
    <div className="splash">
      ...
      <div className="fixed bottom-0 left-0 right-0 flex justify-between p-8">
        <Button onClick={() => handleClick('caully', '/caully')} label="Caully (add animation)" />
        <Button onClick={() => handleClick('fred', '/fred')} label="Fred (add animation)" />
      </div>
    </div>
  );
}
